...
class DroneGymGazeboEnvContinuous(gym.Env):
	def __init__(self, drone_name="drone1", render_mode="human"):
		...
		self.height = 6 #height want image to be after processing (original image = 480)
	...
	'''def _set_action(self, action):
		start = self.move(action[0], action[1])#action[0] is vx, action[1] is vy
		while self.action_duration > time.time() - start: #move at vx in x direction and vy in y direction for self.action_duration
			pass'''
	...
	def preprocess_image(self, image):
		...
		#print('cropped_image = ' + str(cropped_image.shape))
		...
		return_img = np.clip(img_transposed, 0, 1)
		#print('return_img = ' + str(return_img.shape))
	...
	'''def is_inside_workspace(self, current_position):
		...
		if not(is_inside):
			rospy.logwarn(str(self.drone_name)+" has exceeded workspace bounds")
	...
	'''def reset(self):	
		...
		if (self.drone.is_armed() == False):
			...
			self.land_disconnect_drone()
			
			#Pause the simulation
			rospy.loginfo('Pausing simulation')
			self.pause_sim()

			# Disable collisions for the drone
			rospy.loginfo('Disabling collisions for the drone')
			self.disable_collisions(self.drone_name)

			# Gradually move the drone to the starting point
			rospy.loginfo('Moving drone to starting point (on ground)')
			self.move_drone_to_position(self.start_point)

			# Enable collisions for the drone
			rospy.loginfo('Enabling collisions for the drone')
			self.enable_collisions(self.drone_name)
