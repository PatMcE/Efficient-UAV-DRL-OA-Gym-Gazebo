...
if __name__ == "__main__":
    ...
    parser.add_argument("-ep", type=int, default=1500, help="The amount of training episodes, default is 100")
    parser.add_argument("-actor_lr", type=float, default=3e-5, help="Learning rate of adapting the network weights, default is 5e-4")
    parser.add_argument("-critic_lr", type=float, default=3e-5, help="Learning rate of adapting the network weights, default is 5e-4")
    ...
    agent = Agent(state_size=(env.observation_space.shape), action_size=action_size, actor_lr=args.actor_lr,
                  ...
                  hidden_size=args.hidden_size, action_prior="uniform", save_load_dir=save_load_dir)
    ...
    for i_episode in range(1, args.ep + 1):
        ...
        # Check for reaching an average reward of 100 if it hasn’t been recorded yet
        if min_reward_time is None and avg_reward >= 200:
            min_reward_time = time.time() - start_time  # Time to reach avg reward of 100
    ...
    if min_reward_time:
        print("Time taken to reach the minimum reward: {}".format(timedelta(seconds=min_reward_time)))
    else:
