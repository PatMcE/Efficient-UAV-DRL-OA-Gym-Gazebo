...
class DroneGymGazeboEnvDiscrete(gym.Env):
	def __init__(self, drone_name="drone1", render_mode="human"):
		...
		self.height = 1 #height want image to be after processing (original image = 480)
		self.width = 32 #width want image to be after processing (original image = 640)
		self.one_image_shape = (1, self.height, self.width) #(channels, height, width)
		self.observation_space = spaces.Box(low=-1.5, high=6.0, shape=(66,), dtype=np.float32)
	...
	def reset_dist_to_desired_point_and_observations(self):
		...
		self.t_minus1_img_obs = np.zeros(self.one_image_shape, dtype=np.float32).flatten()
		self.position_obs = np.zeros((2,), dtype=np.float32)
	...
	def preprocess_image(self, image):
		# Convert to OpenCV format (assume the image is in 480x640 resolution)
		cv_image = self.depth_imgmsg_to_cv2(image)  # 480, 640

		# Calculate the cropping margins to maintain the center (original 4:3) (new 4:1) 32:1
		original_height = cv_image.shape[0]  # 480 (new 1) #8
		original_width = cv_image.shape[1]   # 640 (new 32)

		new_height = 20 # 120
		crop_top = int((original_height - new_height) // 2)
		crop_bottom = int(crop_top + new_height)

		# Crop the image to 640x240 to maintain 8:3 aspect ratio
		cropped_image = cv_image[crop_top:crop_bottom, :]

		# Normalize, resize, reshape, and transpose
		img_normalized = cv2.normalize(cropped_image, None, 0, 1, cv2.NORM_MINMAX, dtype=cv2.CV_32F)
		#print('img_normalized = ' + str(img_normalized.shape))
		img_resized = cv2.resize(img_normalized, (self.one_image_shape[2], self.one_image_shape[1]), interpolation=cv2.INTER_CUBIC)
		#print('img_resized = ' + str(img_resized.shape))

		img_reshaped = img_resized.reshape((self.one_image_shape[1], self.one_image_shape[2], self.one_image_shape[0]))
		#print('img_reshaped = ' + str(img_reshaped.shape))
		img_transposed = img_reshaped.transpose(2, 0, 1)  # transpose to the form (channel, height, width) for PyTorch
		#print('img_transposed = ' + str(img_transposed.shape))

		return_img = np.clip(img_transposed, 0, 1)
		#print('return_img shape = ' + str(return_img.shape))
	...
	def _get_obs(self):
		gt_pose = self.get_gt_pose()
		self.position_obs[0] = np.float32(round(self.desired_point.x - gt_pose.pose.position.x, 2))
		self.position_obs[1] = np.float32(round(self.desired_point.y - gt_pose.pose.position.y, 2))

		ROS_img_msg = self.get_front_camera_depth_image_raw()
		img_obs = self.preprocess_image(ROS_img_msg)
		img_obs = img_obs.flatten()
		#print('img_obs flatten = ' + str(img_obs.shape))

		combined_img_obs = np.concatenate((img_obs, self.t_minus1_img_obs, self.position_obs), axis=0)
		#print('combined_img_obs = ' + str(combined_img_obs.shape))
	...
	def _is_done(self, observations):
		...
		current_position.x = round(self.desired_point.x - observations[-2], 2) #observation[1][0] = x position observation
		current_position.y = round(self.desired_point.y - observations[-1], 2) #observation[1][1] = y position observation
		current_position.z = self.desired_point.z
		#print('observations[-2] = ' + str(observations[-2]))
		#print('observations[-1] = ' + str(observations[-1]))
		#print('current position x = ' + str(current_position.x))
		#print('current position y = ' + str(current_position.y))
	...
	def reset(self):
		...
		if (self.drone.is_armed() == False):
			...
			rospy.loginfo("Send to x-0.55, y, z")
			cmd = SetPositionWithYawCmdBuilder.build(x=x-0.55, y=y, z=z)
