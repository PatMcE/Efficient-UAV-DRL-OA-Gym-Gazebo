...
class Agent():
    def __init__(self, state_size, action_size, actor_lr, critic_lr, gamma, fixed_alpha, tau, batch_size, buffer_size, hidden_size, action_prior="uniform"):
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        self.gamma = gamma
        self.fixed_alpha = 0.3#None
        self.tau = tau
        ...
        # Actor Network 
        self.actor_local = ActorNetwork(state_size, action_size, self.device, hidden_size).to(self.device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=actor_lr)     
        
        # Critic Network (w/ Target Network)
        self.critic1 = CriticNetwork(state_size, action_size, self.device, hidden_size).to(self.device)
        self.critic2 = CriticNetwork(state_size, action_size, self.device, hidden_size).to(self.device)
        
        self.critic1_target = CriticNetwork(state_size, action_size, self.device, hidden_size).to(self.device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = CriticNetwork(state_size, action_size, self.device, hidden_size).to(self.device)
        ...
        # Replay memory
        self.memory = ReplayBuffer(buffer_size, state_size, action_size)#self.memory = ReplayBuffer(action_size, buffer_size, batch_size, self.device)
