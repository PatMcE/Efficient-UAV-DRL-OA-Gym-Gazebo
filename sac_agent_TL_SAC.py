...
class Agent():
    def __init__(self, state_size, action_size, actor_lr, critic_lr, gamma, fixed_alpha, tau, batch_size, buffer_size, hidden_size, save_load_dir=None, action_prior="uniform"):
        ...
        # Actor Network 
        self.actor_local = ActorNetwork(state_size, action_size, self.device, hidden_size, pretrained_path = self.save_load_dir+'actor_old.pth').to(self.device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=actor_lr)     
        
        # Critic Network (w/ Target Network)
        self.critic1 = CriticNetwork(state_size, action_size, self.device, hidden_size, pretrained_path = self.save_load_dir+'critic1_old.pth').to(self.device)
        self.critic2 = CriticNetwork(state_size, action_size, self.device, hidden_size, pretrained_path = self.save_load_dir+'critic2_old.pth').to(self.device)
        
        self.critic1_target = CriticNetwork(state_size, action_size, self.device, hidden_size, pretrained_path = self.save_load_dir+'critic1_target_old.pth').to(self.device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = CriticNetwork(state_size, action_size, self.device, hidden_size, pretrained_path = self.save_load_dir+'critic2_target_old.pth').to(self.device)
    ...
    def learn(self, step, d=1):
        ...
        #print('## learn Q_target_next = ' + str(Q_target_next.shape))
